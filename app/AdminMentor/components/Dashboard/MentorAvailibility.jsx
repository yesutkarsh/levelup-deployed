  ...
  return (
    <div className="mt-8 bg-white p-8 rounded-2xl border border-gray-100 shadow-lg">
      ...
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-2xl font-bold text-gray-900">Mentor Availability Settings</h2>
        <button 
          onClick={handleSave}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">Break Between Sessions</label>
                <select
                  value={breakDuration}
                ...
                >
                  {[5, 10, 15, 30].map((breakTime) => (
                    <option key={breakTime} value={breakTime}>{breakTime} minutes</option>
                  ))}
                </select>
              </div>
              <div className="flex items-center justify-between pt-2">
                <span className="text-sm font-medium text-gray-700">Allow Booking Buffer</span>
          ...
          {/* Slots Configuration */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">Slots Configuration</h3>
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">Day</label>
                <select
                  value={newSlotConfig.day}
                  onChange={(e) => setNewSlotConfig(prev => ({...prev, day: e.target.value}))}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all outline-none"
                >
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">Number of Slots</label>
                <select
                  value={newSlotConfig.slots}
                  onChange={(e) => setNewSlotConfig(prev => ({...prev, slots: Number(e.target.value)}))}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all outline-none"
                >
                  {[...Array(10).keys()].map((num) => (
                    <option key={num} value={num + 1}>{num + 1} slot{num > 0 ? "s" : ""}</option>
                  ))}
                </select>
              </div>
            </div>
            <button 
              ...
              className="w-full mt-3 px-4 py-2.5 text-sm font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-all duration-200"
            >
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {slots.map((slot, index) => (
                    <tr key={index} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3.5 text-sm font-medium text-gray-900">{slot.day}</td>
                      <td className="px-4 py-3.5 text-sm text-gray-600">{slot.slots} slots</td>
