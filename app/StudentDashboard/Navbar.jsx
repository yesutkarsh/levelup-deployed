...
export default function Navbar() {
  ...
  const handleNavChange = (item) => {
    ...
    if (item !== "Schedule") setSelectedSession(null);
    setActiveNav(item);
  };
  ...
  const renderComponent = () => {
    switch (activeNav) {
      case "Dashboard":
        return <StudentDashboard onNavChange={handleNavChange} />;
      case "Schedule":
        return selectedSession ? (
          ...
          <SessionsPage
            ...
            onJoinSession={(session) => alert(`Joining ${session.title}`)}
            onWriteReview={(session) =>
              alert(`Writing review for ${session.title}`)
            }
          />
        );
      case "Resources":
